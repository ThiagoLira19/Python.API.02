...
@app.route("/item/search/<produto>", methods=["GET"])
def get_produto(produto):
    ...
    try:
        ...
        # Executa o SQL.
        cursor.execute(
            "SELECT * FROM item WHERE item_status != 'off' AND (item_name LIKE '%' || ? || '%' OR item_description like '%'||?||'%' OR item_location like '%'||?||'%');", (produto,produto,produto,))
...
@app.route("/item/all_proprietario/<int:id>", methods=["GET"])
def get_produto_all_owner(id):
    ...
    try:
        ...
        # Executa o SQL.
        cursor.execute(
            "select * from owner left join item on item_owner=owner_id WHERE owner_id= ?;", (id,))
...
@app.route("/item/all/<int:id>", methods=["GET"])
def get_produto_all(id):
    ...
    try:
        ...
        # Executa o SQL.
        cursor.execute(
            "select * from item a inner join owner on item_owner=owner_id WHERE item_id= ?;", (id,))
